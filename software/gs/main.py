...
print(f"Loop: {gs.counter}, Total Msgs: {gs.msg_count}, Msgs in Cache: {gs.msg_cache}, Vbatt: {gs.battery_voltage}")

# try connecting to wifi
print("Connecting to WiFi...")
try:
    wifi.radio.connect(secrets["ssid"], secrets["password"]) # open network
    print("Network opened!")
    print(f"Signal:{wifi.radio.ap_info.rssi}")
    ...
    for r in radios:
        if gs.rx_done(radios[r]):
            ...
            for msg in gs.get_msg2(radios[r]):
                if msg is not None:
                    print(f"[{bytes(msg)}] rssi: {gs.last_rssi}", end = ", ")
...
# if we can't connect, cache message
    ...
    for msg in new_messages:
        ...
        try:
            ...
            print("Can't cache msg. Connected to USB?")
        gs.msg_cache=gs.msg_cache+1

gs.counter = gs.counter + 1

print(f"Finished. Deep sleep until RX interrupt or {gs.deep_sleep}s timeout...")
